...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '30f8b218-eee1-40fb-a761-c9e9fe0abbcd'; // ID de Sheena Studio
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'Sheena Studio';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '54151612';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'danelys961@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Sheena Studio!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'sheenastudio';
};
...
console.log('✅ config-negocio.js listo para Sheena Studio');
